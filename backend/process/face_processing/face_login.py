...
class FaceLogIn:
    ...
    def process(self, face_image: np.ndarray, username: str = None):
        # Cargar la imagen del usuario almacenada
        stored_face = self.face_utilities.load_user_face_image(username)
        ...
        if stored_face is not None:
            ...
            self.matcher, username1, distance = self.face_utilities.face_matching(face_crop, stored_face, username)

            if self.matcher:
                check_user_in, info_check = self.face_utilities.check_in_user(username, distance)
                return face_crop, True, {'username': username,'distance': distance}
